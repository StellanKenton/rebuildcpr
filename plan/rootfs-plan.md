...
目标是在当前 MCU 工程里做一个简化版、类 Linux 使用方式的 filesystem 抽象层，后续统一放在 `rep/service/vfs/` 下。
...
所以后续方向不是继续扩展 `User/manager/memory`，而是把其中可复用的语义上提到 `rep/service/vfs/`，项目绑定部分留在 `User/`。
...
4. `rep/service/vfs/` 只放可复用 core 和 backend 适配，不放具体板级绑定。
...
- `vfs.md`
  作为 `rep/service/vfs/` 的目录权威文档，写明边界、依赖、初始化顺序、路径语义和项目侧接入方式。
- `vfs_cfg.h`
...
- 启动初始化和调试注册接入。
- `rep/service/vfs/vfs.md` 与上层目录说明同步更新。
...
1. 新建 `rep/service/vfs/` 目录和权威文档，先把路径模型、挂载模型、句柄模型、错误码模型写死。
...
10. 更新 `rep/rule/map.md`，补齐 `rep/service/vfs/` 的目录定位和边界说明；如最终归到 `service` 风格，也要同步修正文档边界。
...
- 仓库中已经存在并接线完毕 `rep/service/vfs/`、`User/port/`、`User/manager/memory/`、`User/system/` 的对应文件。
...
3. 不要把具体 flash 型号、SPI bus、CS 引脚、分区地址写死在 `rep/service/vfs/` core。
...
1. 新增 `rep/service/vfs/vfs.md` 作为该目录权威入口文档。
2. 更新上层入口文档，至少补 `rep/rule/map.md` 对 `vfs/` 的说明。
3. 如果最后把 `vfs` 定位成 `service` 风格能力，也要同步补充与 `rep/service/service.md` 的边界说明，避免目录语义漂移。
